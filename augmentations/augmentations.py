...
def extractFeaturesAndDelete(filename):
    os.system("python ./feature_extraction.py " + outfile_path + " ./output.csv")
    os.unlink(filename)
...
def random_cropping(infile, minLength = 1):
    ...
    if (endTime > minLength):
        ...
        writer = es.MonoWriter(filename = outfile)
        writer(y[0])
        if (FILE_DELETION):
...
def add_noise(infile, noise_name, snr):
    ...
    x = loader1()
    noise_path = '/Users/shiva/Study/internship/experiments/augmentations/sounds/%s.wav' % noise_name
...
def convolve(infile, ir_name, level = 0.5):
    ...
    ir_path = '/Users/shiva/Study/internship/experiments/augmentations/sounds/ir_{0}.wav'.format(ir_name)
    loader2 = es.MonoLoader(filename = ir_path)
    ir = loader2()
    y = np.convolve(x, ir, 'full')[0:x.shape[0]] * level + x * (1 - level)

    #Change the output file name to suit your requirements here
    outfile_name = os.path.basename(infile).split(".")[0] + ("_convolved%s.wav" % str(level))
    outfile = os.path.join(outfile_path, outfile_name)
    writer = es.MonoWriter(filename = outfile)
    writer(y)
    if (FILE_DELETION):
...
def apply_rubberband(infile, time_stretching_ratio=1.0, pitch_shifting_ratio=1.0):
    ...
    y = loader2()
    #Change the output file name to suit your requirements here
    outfile_name = os.path.basename(infile).split(".")[0] + ("_timestr%s_pitchshift%s.wav" % (str(time_stretching_ratio),str(pitch_shifting_ratio)))
    outfile = os.path.join(outfile_path, outfile_name)
    writer = es.MonoWriter(filename = outfile)
    writer(y)
    if (FILE_DELETION):
